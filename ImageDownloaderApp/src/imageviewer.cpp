...
void ImageViewer::paintEvent(QPaintEvent *)
{
    ...
    {
        p.drawPixmap(
            QRect(-m_screenW / 2, -m_screenH / 2, m_screenW, m_screenH),
            m_cachedPixmap);
    }
    else
    {
...
void ImageViewer::mouseMoveEvent(QMouseEvent *e)
{
    ...
    {
        m_xMov = std::max(
            std::min(m_lastPt.x() - xxf, m_imagePtr->width() - m_cW), 0);
        m_yMov = std::max(
            std::min(m_lastPt.y() - yyf, m_imagePtr->height() - m_cH), 0);
...
void ImageViewer::queueGenerateCache()
{
    ...
    if (!cacheTimer_)
    {
        ...
        connect(cacheTimer_, &QTimer::timeout, this,
                &ImageViewer::generateCache);
    }
    if (cacheTimer_)
